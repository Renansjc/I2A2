<template>
  <div class="space-y-6">
    ...
    <div v-if="currentView === 'upload'" class="card bg-base-200 shadow-lg">
      <div class="card-body">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex items-center space-x-2">
            <div class="text-sm text-base-content/60">
              {{ queueStats.processing }} processando • {{ queueStats.completed }} concluídos
            </div>
            <button 
        ...
        <div v-if="processingQueue.length === 0" class="text-center py-8">
          ...
          <p class="text-sm text-base-content/40 mt-2">Arquivos enviados aparecerão aqui para acompanhamento em tempo real</p>
        </div>
        ...
        <div v-else class="space-y-3">
          <div 
            ...
            :key="item.id"
            class="flex items-center justify-between p-4 bg-base-100 rounded-lg hover:shadow-md transition-shadow cursor-pointer"
            @click="viewDocumentDetails(item.id)"
          >
            <div class="flex items-center space-x-4 flex-1">
              <div class="w-10 h-10 rounded-lg flex items-center justify-center"
                   :class="{
                     'bg-blue-100 text-blue-600': item.type === 'NF-e',
                     'bg-green-100 text-green-600': item.type === 'NFS-e',
                     'bg-gray-100 text-gray-600': !item.type
                   }">
              ...
              <div class="flex-1 min-w-0">
                <h4 class="font-semibold truncate">{{ item.filename }}</h4>
                <div class="flex items-center space-x-2 text-sm text-base-content/70">
                  <span>{{ item.type || 'XML' }}</span>
                  <span>•</span>
                  <span>{{ item.size }}</span>
                  <span v-if="item.emitente" class="text-primary">• {{ item.emitente }}</span>
                </div>
                <div class="text-sm text-base-content/50 mt-1">
                  {{ item.currentStep || getStatusDescription(item.status) }}
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center space-x-4">
              ...
              <div v-if="item.agentStatuses" class="flex space-x-1">
                <div
                  v-for="agent in item.agentStatuses"
                  :key="agent.name"
                  class="w-2 h-2 rounded-full"
                  :class="{
                    'bg-gray-300': agent.status === 'pending',
                    'bg-blue-500 animate-pulse': agent.status === 'in_progress',
              ...
              <div 
                class="badge"
                :class="{
                  'badge-warning': item.status === 'processing' || item.status === 'pending',
                  'badge-success': item.status === 'completed',
                  'badge-error': item.status === 'error' || item.status === 'failed',
